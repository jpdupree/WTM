...
// WTM scores every lap as a flat official distance (5 mi) regardless of the
// GPS-measured polyline length. Pass --lap-miles 5 to emit that as LAP_MILES
// and rescale every per-point / per-obstacle mile into official miles, so the
// map geometry and the results feed's distances share one unit. Omit it to
// keep the raw measured distances.
const officialLapArg = arg("--lap-miles");
const officialLap = officialLapArg !== undefined ? parseFloat(officialLapArg) : null;
...
const measuredMiles = cum[cum.length - 1] / M_PER_MILE;

// If an official lap length was given, scale measured distances into
// official miles; otherwise report measured distances unchanged.
const lapMiles = officialLap || measuredMiles;
const mileScale = officialLap ? officialLap / measuredMiles : 1;
const toMile = (meters) => (meters / M_PER_MILE) * mileScale;

// Course as lat/lng points, each carrying its cumulative mile.
const course = loop.coords.map(([lon, lat], i) => ({
  lat: +lat.toFixed(6),
  lng: +lon.toFixed(6),
  mile: +toMile(cum[i]).toFixed(4),
}));
...
for (const p of placemarks) {
  ...
  obstacles.push({
    ...
    mile: +toMile(cum[best]).toFixed(3),
  });
}
...
// Full lap length in miles${officialLap ? ` (official; GPS-measured ${measuredMiles.toFixed(3)})` : ""}.
export const LAP_MILES = ${lapMiles.toFixed(3)};
...
const nameSrc =
  kmlNamedCount === obstacles.length
    ...
    : kmlNamedCount > 0
      ...
      : "fallback";
const lapNote = officialLap
  ? `lap ${lapMiles.toFixed(2)} mi official (measured ${measuredMiles.toFixed(2)})`
  : `lap ${measuredMiles.toFixed(2)} mi measured`;
console.log(
  `course: ${lapNote}, ${course.length} points, ` +
